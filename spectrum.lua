...
Player = plr 
function RequestHit()
	script.Mouse:FireClient(Player)
end
char = plr.Character
...
a.Transparency = 1
	...
	local ray = Ray.new(
	    dis.CFrame.p,                           -- origin
	    (Hit.p - dis.CFrame.p).unit * 500 -- direction
	) 
	local ignore = dis
...
a.Transparency = 1
	...
	local ray = Ray.new(
	    ...
	    (Hit.p - cent.CFrame.p).unit * 500 -- direction
	) 
	local ignore = cent
	local hit, position, normal = workspace:FindPartOnRay(ray, ignore)
	a.BottomSurface = 10
...
local attacktype = 1
mouse.OnServerEvent:connect(function(play,ty,k,hit)
	Hit = hit
	if ty == "mb1c" then
...
newThemeCust("rbxassetid://1359036559",0,1.01,1.5)
...
chaosmode = false
CRAZED = false
RecolorTextAndRename("Love",BrickColor.new("White").Color,BrickColor.new("Bright violet").Color,"Code")
newTheme("rbxassetid://1426044282",0,1,1.15)
MAINRUINCOLOR = BrickColor.new("Bright violet")
RecolorThing(MAINRUINCOLOR,BrickColor.new("Bright violet"),MAINRUINCOLOR,MAINRUINCOLOR,MAINRUINCOLOR,1,MAINRUINCOLOR,1,MAINRUINCOLOR,false,false)
end
...
swait()
if ModeOfGlitch == 102 then
sphereMK(7.5,math.random(-50,-15)/45,"Add",root.CFrame*CFrame.new(math.random(-25,25),50,math.random(-25,25))*CFrame.Angles(math.rad(90 + math.random(-20,20)),math.rad(math.random(-20,20)),math.rad(math.random(-20,20))),0.75,0.75,10,-0.0075,BrickColor.new("Bright violet"),0)
end
end
end))
